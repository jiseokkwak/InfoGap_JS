...
def main(args):
    ...
    # preprocessor_without_normalize contains:
    ...
    with torch.no_grad():
        ...
        for el in (text_tokens[:500], text_tokens[500:]):
            # Split to avoid OOM; model_orig.encode_text normalizes the embeddings
    ...
    # Save the text encoder for loss calculation (CLIP의 텍스트 인코딩 기능 사용)
    clip_text_encoder = model_orig
    clip_text_encoder = clip_text_encoder.to(main_device)
    model_orig.cpu()
    ...
    while step_total < args.steps:
        step_total = train_one_epoch(
            ...
            args=args,
            epoch=epoch,
            clip_text_encoder=clip_text_encoder,  # 추가: 텍스트 임베딩용 CLIP 모델
            text_tokens=text_tokens             # 추가: 텍스트 토큰
        )
...
# 수정된 ComputeLossWrapper: text_tokens와 텍스트 인코딩 모델을 추가 인자로 받음
class ComputeLossWrapper:
    def __init__(self, embedding_orig, embedding_text_labels_norm, text_tokens, model_orig_for_text,
                 reduction='mean', loss=None, logit_scale=100.):
        self.embedding_orig = embedding_orig
        self.embedding_text_labels_norm = embedding_text_labels_norm
        self.text_tokens = text_tokens
        self.model_orig_for_text = model_orig_for_text
    ...
    def __call__(self, embedding, targets):
        return compute_loss(
            ...
            model_orig_for_text=self.model_orig_for_text,
            reduction=self.reduction
        )


def l2(out, text_tokens, targets, model_orig, reduction='none'):
    """
    out: 이미지 임베딩, shape (B, D)
    text_tokens: 전체 클래스에 대한 텍스트 토큰, shape (num_classes, n_ctx)
    targets: 각 이미지의 정답 클래스 인덱스, shape (B,)
    model_orig: 텍스트 인코딩을 위한 CLIP 모델
    reduction: 'mean', 'sum', 'none'
    """
    # 모델과 텍스트 토큰을 out과 같은 디바이스로 옮깁니다.
    model_orig = model_orig.to(out.device)
    text_tokens = text_tokens.to(out.device)
    
    # 전체 텍스트 임베딩 계산, shape: (num_classes, D)
    with torch.no_grad():
        all_text_embeddings = model_orig.encode_text(text_tokens.to(out.device), normalize=True)
    
    # 각 이미지에 해당하는 정답 텍스트 임베딩 선택, shape: (B, D)
    # targets가 클래스 인덱스여야 하며, long 타입이어야 합니다.
    selected_text_embeddings = all_text_embeddings[targets.long()]
    
    # 이미지 임베딩과 선택된 텍스트 임베딩 사이의 L2 손실 계산
    l2_out_text = F.mse_loss(out, selected_text_embeddings, reduction='none').sum(dim=1)
    
    # 만약 clean embedding(embedding_orig)와의 차이를 계산하려면,
    # selected_text_embeddings와 embedding_orig (정답의 클린 임베딩)이 같은 shape (B, D)여야 합니다.
    l2_text_targets = F.mse_loss(selected_text_embeddings, targets, reduction='none').sum(dim=1)
    
    loss = -l2_out_text + l2_text_targets
    if reduction == 'mean':
        return loss.mean()
    elif reduction == 'sum':
        return loss.sum()
...
# 수정된 compute_loss: l2 손실 계산 시 text_tokens와 텍스트 인코딩 모델을 전달
def compute_loss(loss_str, embedding, targets, embedding_orig, logit_scale,
                 embedding_text_labels_norm=None, text_tokens=None, model_orig_for_text=None, reduction='mean'):
    if loss_str == 'l2':
        loss = l2(out=embedding, text_tokens=text_tokens, targets=targets,
                  model_orig=model_orig_for_text, reduction=reduction)
    elif loss_str == 'ce':
        loss = ce(
...
def train_one_epoch(
        ...
        embedding_text_labels_norm, args, epoch, dataloader_eval=None, clip_text_encoder=None, text_tokens=None
):
    model_orig.eval()
    model.train()
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        # ComputeLossWrapper에 text_tokens와 clip_text_encoder 전달
        loss_inner_wrapper = ComputeLossWrapper(
            embedding_orig, embedding_text_labels_norm, text_tokens, clip_text_encoder,
        ...
        loss = compute_loss(
            ...
            text_tokens=text_tokens, model_orig_for_text=clip_text_encoder,
            reduction='mean'
        )
        ...
        if (step_total-1) % args.log_freq == 0:
            ...
            if (step_total-1) % (args.log_freq * 10) == 0:
                ...
                this_epoch_remaining = epoch_average_time - (time.time() - epoch_start_time) / 60**2
                total_remaining = epoch_average_time * (args.total_epochs - epoch - i / len(dataloader))
                print(f'[epoch average time] {epoch_average_time:.2f} [this epoch remaining] {this_epoch_remaining:.2f} [total remaining] {total_remaining:.2f}')
                log_data.update({
