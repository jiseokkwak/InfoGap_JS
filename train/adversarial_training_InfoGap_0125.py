...
class ComputeInfoGapLossWrapper:
    def __init__(self, y, clean_embedding, discriminator, mi_estimator_weighted, mi_estimator_standard, lambda_val):
        """
        y: 텍스트 임베딩 (shape [batch_size, embed_dim])
        clean_embedding: clean 이미지 임베딩 (p(x,y)에서 뽑은 x)
        discriminator: 이미 freeze된 discriminator 모델
        mi_estimator_weighted: 수정된 WeightedMine 모듈
        mi_estimator_standard: 기존의 Mine 모듈
    ...
    def __call__(self, embedding_adv, targets=None):
        # Discriminator를 통해 w_q 계산 (adversarial embedding 기반)
        logits_q = self.discriminator(embedding_adv, self.y).squeeze()
        D_psi_q = torch.sigmoid(logits_q)
        w_q = D_psi_q / (1.0 - D_psi_q + 1e-8)  # 안정성을 위해 작은 값 추가

        # Weighted MI 계산 시, clean embedding을 함께 사용
        weighted_mi, t_clean, t_shuffled, first_term, second_term = self.mi_estimator_weighted(
            embedding_adv, self.clean_embedding, self.y, w_q
        )
        # Standard MI 계산은 그대로 adversarial embedding으로 계산
        standard_mi, _, _, first_term_std, second_term_std = self.mi_estimator_standard(embedding_adv, self.y)
...
class Mine(nn.Module):
    ...
    def forward(self, x, y):
        ...
        mi_estimate = first_term # - second_term
        # intermediate 값도 반환
...
class WeightedMine(nn.Module):
    ...
    def forward(self, x_adv, x_clean, y, w):
        # 첫 번째 항: clean embedding을 사용하여 T(x_clean, y) 계산
        t_clean = self.T(x_clean, y)
        first_term = torch.mean(t_clean)

        # 두 번째 항: adversarial embedding에서 y를 셔플한 뒤, weight 적용
        perm = torch.randperm(y.size(0))
        y_shuffled = y[perm]
        w_shuffled = w[perm]
        t_shuffled = self.T(x_adv, y_shuffled)
        ...
        mi_estimate = first_term #- second_term
        # 중간 값들을 필요에 따라 반환합니다.
        return mi_estimate, t_clean.detach(), t_shuffled.detach(), first_term.detach(), second_term.detach()
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        loss_inner_wrapper = ComputeInfoGapLossWrapper(
            y=y,
            clean_embedding=embedding_orig,
            discriminator=discriminator,
            mi_estimator_weighted=mi_estimator_weighted,
            mi_estimator_standard=mi_estimator_standard,
            lambda_val=args.lambda_val
        ...
        weighted_mi, t_clean, t_shuffled, first_term, second_term = mi_estimator_weighted(embedding_adv, embedding_orig, y, w_q)
       ...
       #metrics['w_q'].append(w_q_quartile_mean)
        ...
        print(f"T(x_clean, Y) mean: {t_clean.mean().item():.4f}, First term (mean of T(x_clean,Y)): {first_term.item():.4f}, T(x_adv, Y_shuffled) mean: {t_shuffled.mean().item():.4f}")
