'''
패치노트 (2025.0202)
1. discriminator 더 작게 만들음 (latent * 2 -> 512 -> 512 ->1 에서 중간 512 layer 제거)
2. discriminator 에 대해서 gradient panelty 추가
'''
...
def compute_gradient_penalty(discriminator, real_data, fake_data, y, device, penalty_coef=10):
    batch_size = real_data.size(0)
    alpha = torch.rand(batch_size, 1, device=device).expand_as(real_data)
    interpolates = alpha * real_data + (1 - alpha) * fake_data
    interpolates.requires_grad_(True)
    
    disc_interpolates = discriminator(interpolates, y)
    ...
    penalty = penalty_coef * ((gradient_norm - 1) ** 2).mean()
    return penalty
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        gp = compute_gradient_penalty(discriminator, embedding_orig.detach(), embedding_adv.detach(), y.detach() ,device, penalty_coef=10)
        loss_d += gp
