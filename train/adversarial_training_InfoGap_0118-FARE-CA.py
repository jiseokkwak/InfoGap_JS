...
parser.add_argument('--t_network_type', type=str, default='cossim', choices=['cossim', 'l2norm'], help='Type of T network for MI estimation: cossim or l2norm') # 새 인자 추가
...
class ComputeInfoGapLossWrapper:
    ...
    def compute_gradient_norm(self, loss, target_layer_params): # Changed target_layer to target_layer_params
        # Temporarily enable gradients for the requested parameters
        orig_flags = [p.requires_grad for p in target_layer_params]
        for p in target_layer_params:
            p.requires_grad_(True)

        # Compute gradients w.r.t. target_layer_params
        grads = torch.autograd.grad(
            loss,
            target_layer_params,
            retain_graph=True,
            create_graph=True,
            allow_unused=False, # Ensure all target_layer_params are used or error
        )
        ...
        for p, flag in zip(target_layer_params, orig_flags):
            p.requires_grad_(flag)
            if p.grad is not None: # Clear gradients that might have been set
                p.grad.zero_()
    ...
    def __call__(self, embedding_adv, targets, step=None, total_steps=None):
        ...
        # Gradient Norm 계산
        # Use the classifier parameters for gradient norm calculation
...
class T(nn.Module):
    def __init__(self, x_dim, y_dim, t_network_type='cossim', temperature=1.0): # x_dim, y_dim 추가 및 temperature 기본값 설정
        super().__init__()
...
class WeightedMine(nn.Module):
    ...
    def forward(self, x, y, w):
        ...
        y_shuffled = y[perm]
        #w_shuffled = w[perm]
        w_shuffled = compute_w(x, y_shuffled, self.discriminator)  # w_shuffled 계산
        t_shuffled = self.T(x, y_shuffled)
        ...
        #second_term = torch.log(exp_mean + EPS)  # 직접 exp_mean 사용
        second_term = torch.log((w_shuffled * torch.exp(t_shuffled)).mean() + EPS)  # w_shuffled와 t_shuffled를 곱한 후 평균 계산
        mi_estimate = first_term - second_term * args.second_term_coeff
...
class Discriminator(nn.Module): # New CrossAttnDiscriminator
    """
    ...
    def __init__(
        ...
        # num_heads: int = 4, # Will get from args
        # mlp_dim: int = 512, # Will get from args
        # dropout: float = 0.1, # Will get from args
        # use_spectral_norm: bool = False, # Will get from args
    ):
        super().__init__()
        # y_dim is passed but for this specific cross-attention model,
        # it's assumed x and y embeddings have the same dimension `dim`.
        if dim != y_dim:
            print(f"Warning: CrossAttnDiscriminator received x_dim={dim} and y_dim={y_dim}. Using x_dim for internal dimension.")

        num_heads = args.disc_num_heads
        mlp_dim = args.disc_mlp_dim
        dropout_rate = args.dropout # Using the global dropout from args
        ...
        self.ffn = nn.Sequential(
            linear1,
            nn.LeakyReLU(0.1, inplace=True), # LeakyReLU from original FFN
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 시퀀스 길이 1인 경우 [B, 1, D] 로 확장
        if x.dim() == 2:
            x = x.unsqueeze(1)
        if y.dim() == 2:
            y = y.unsqueeze(1)

        # (1) Cross-Attention + 잔차
        q_in = self.pre_ln(x) # Query from x
        kv_in = self.pre_ln(y) # Key/Value from y
        attn_out, _ = self.cross_attn(q_in, kv_in, kv_in)  # [B, Sx, D]
        x_res = x + attn_out              # 첫 번째 잔차

        # (2) FFN + 잔차
        z_in = self.post_attn_ln(x_res)
        z_out = self.ffn(z_in)               # [B, Sx, D]
        x_res = x_res + z_out                     # 두 번째 잔차

        # (3) Pool & 분류
        # Taking mean over the sequence dimension (dim=1) that came from x
        logits = self.classifier(x_res.mean(dim=1)) # .squeeze(1) is not needed if output is [B,1]
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        # 2) Text embedding
        y = embedding_text_labels_norm[:, targets].T  # shape: (batch_size, embed_dim) # This y is still needed for outer minimization
        ...
        loss_inner_wrapper = ComputeLossWrapper(
            ...
            reduction='mean',
            loss='ce', # Use Cross-Entropy loss for inner maximization
            logit_scale=1.0 / args.temperature # Corresponds to 1/tau
        )
        ...
        # trainable_modules = [discriminator.classifier]
        # frozen_modules = [
        #     discriminator.pre_ln, 
        #     discriminator.cross_attn, 
        #     discriminator.ffn, 
        #     discriminator.post_attn_ln
        # ]
...
def main():
    ...
    # discriminator.apply(weights_init) # Removed for CrossAttnDiscriminator
    ...
    T_network = T(x_dim, y_dim, t_network_type=args.t_network_type, temperature=args.temperature).to(device) # t_network_type 및 temperature 전달
