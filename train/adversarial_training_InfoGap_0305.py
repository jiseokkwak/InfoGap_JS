...
parser.add_argument('--coeff_second', type=float, default=1, help='Coefficient for second term in MINE')
parser.add_argument('--use_spectral_norm', type=str2bool, default=False, help='Use spectral normalization in discriminator')
...
class ComputeInfoGapLossWrapper:
    def __init__(self, y, discriminator, mi_estimator_weighted, mi_estimator_standard, lambda_val):
        ...
        self.return_components = False  # 기본값 추가
    ...
    def __call__(self, embedding_adv, targets=None):
        ...
        # return_components가 True면 딕셔너리 반환, 아니면 손실값만 반환
        if self.return_components:
            return {
                ...
                'secondterm': weighted_mi - standard_mi  # 그래디언트 계산에 필요한 항목
            }
        else:
            return loss
...
class Mine(nn.Module):
    ...
    def forward(self, x, y):
        ...
        mi_estimate = first_term - args.coeff_second * second_term
        # intermediate 값도 반환
