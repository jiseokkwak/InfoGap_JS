...
#gradient penalty + spcetral normalization 미적용(clipping 제외)
#first term에서 간소화된 loss 사용 
import sys
import os
import shutil
import time
...
parser.add_argument('--trainable_layers', type=str, default='all', 
                    help="Which layers to train: 'all', 'last', 'last_n' (e.g., 'last_3'), or list of indices")
...
class ComputeInfoGapLossWrapper:
    def __init__(self, y, discriminator, mi_estimator_weighted, mi_estimator_standard, lambda_val, T_network):
        """
    ...
    def __call__(self, embedding_adv,targets=None):
        ...
        """
        # 1) 기존 방식: 1:1 대응으로 discriminator 통과
        logits_q = self.discriminator(embedding_adv, self.y).squeeze()  # 예상 shape: [B] (B=batch size)
        D_psi_q = torch.sigmoid(logits_q)
        w_q = D_psi_q / (1.0 - D_psi_q + 1e-8)  # shape: [B]
        E_q_wlog_w = (-1+w_q+w_q * torch.log(w_q + 1e-8)).mean()

        # 2) 추가 방식: 배치 내의 모든 256×256 조합 계산
        B = embedding_adv.size(0)
        # 확장: (B, d) -> (B, 1, d) 및 (B, d) -> (1, B, d)
        adv_exp = embedding_adv.unsqueeze(1).expand(-1, B, -1)  # [B, B, d]
        y_exp = self.y.unsqueeze(0).expand(B, -1, -1)            # [B, B, d]
        # 각 쌍에 대해 discriminator 계산: 각 쌍은 (adv_i, y_j)
        adv_flat = adv_exp.contiguous().view(B * B, -1)  # [B*B, d]
        y_flat = y_exp.contiguous().view(B * B, -1)      # [B*B, d]
        logits_pair = self.discriminator(adv_flat, y_flat).squeeze()  # [B*B]
        D_psi_pair = torch.sigmoid(logits_pair)
        w_pair = D_psi_pair / (1.0 - D_psi_pair + 1e-8)  # [B*B]
        
        t_pair = self.T_network(adv_flat, y_flat)  # shape: [B*B]; 여기서 T(x,y)를 계산
        # t_pair 및 w_pair를 B x B 행렬로 재구성
        t_matrix = t_pair.view(B, B)      # [B, B]
        w_matrix = w_pair.view(B, B)      # [B, B]

        # 행별 계산 (각 x_i에 대해)
        exp_t_matrix = torch.exp(t_matrix)                      # [B, B]
        w_exp_t_matrix = w_matrix * exp_t_matrix                # [B, B]

        # 각 행별로 분자와 분모 계산
        numerator_per_row = w_exp_t_matrix.sum(dim=1)           # [B]
        denominator_per_row = exp_t_matrix.sum(dim=1)           # [B]

        # 각 행별로 log(numerator/denominator) 계산
        log_ratios = torch.log(numerator_per_row / (denominator_per_row + EPS) + EPS)  # [B]

        # 모든 행에 대한 평균 계산
        secondterm = -1.2* log_ratios.mean()
        # w_matrix 계산 직후에 최댓값과 최솟값 출력 추가

        # 최종 InfoGap 손실: 기존 MI 항과 KL-like 항(두 항의 합)
        ...
        loss = - standard_mi + self.lambda_val * (E_q_wlog_w) + secondterm
        return loss
...
class Discriminator(nn.Module):
    def __init__(self, x_dim, y_dim):
        ...
        self.layers = nn.Sequential(
            nn.Linear(x_dim + y_dim, 1024),  # 첫 번째 선형 레이어
            nn.Dropout(args.dropout),
            nn.LeakyReLU(args.leaky_relu, inplace=True),
            nn.Linear(1024, 512),  # 두 번째 선형 레이어
            nn.Dropout(args.dropout),
            nn.LeakyReLU(args.leaky_relu, inplace=True),
            nn.Linear(512, 1),  # 세 번째 선형 레이어
        )
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        if not args.loss_phi_abs:
            loss_phi = loss_inner_wrapper(embedding_adv)
        else:
            loss_phi = (loss_inner_wrapper(embedding_adv))**2
...
# --- Evaluation block ---
        ...
        if args.save_checkpoints and (step_total % (args.steps // 2) == 0):
            os.makedirs(checkpoint_dir, exist_ok=True)
            model_path = os.path.join(checkpoint_dir, f'step_{step_total}.pt')
            optimizer_path = os.path.join(checkpoint_dir, f'step_{step_total}_opt.pt')
            torch.save(unwrap_model(model).state_dict(), model_path)
            torch.save(optimizer.state_dict(), optimizer_path)
        if step_total % (args.steps // 10) == 0: 
...
def main():
    ...
    # Set which layers should be trainable
    set_clip_layers_trainable(model, args.trainable_layers)

    # Only pass trainable parameters to the optimizer
    params = filter(lambda p: p.requires_grad, unwrap_model(model).parameters())
    if args.opt == 'adamw':
...
def set_clip_layers_trainable(model, trainable_layers='last'):
    """
    ...
    vision_model = unwrap_model(model).model
    
    # For ViT models, the structure is typically:
    # - Patch embedding (vision_model.conv1, vision_model.class_embedding, etc.)
    # - Transformer blocks (vision_model.transformer.resblocks)
    # - Final layer norm and projection (vision_model.ln_post, vision_model.proj)
    
    if hasattr(vision_model, 'transformer') and hasattr(vision_model.transformer, 'resblocks'):
        transformer_blocks = vision_model.transformer.resblocks
        num_blocks = len(transformer_blocks)
        
        # Determine which blocks to make trainable
        trainable_indices = []
        
        if trainable_layers == 'all':
            trainable_indices = list(range(num_blocks))
        elif trainable_layers == 'last':
            trainable_indices = [num_blocks - 1]
        elif isinstance(trainable_layers, list):
            trainable_indices = trainable_layers
        elif trainable_layers.startswith('last_'):
            try:
                n_layers = int(trainable_layers.split('_')[-1])
                trainable_indices = list(range(num_blocks - n_layers, num_blocks))
            except:
                raise ValueError(f"Invalid trainable_layers format: {trainable_layers}")
        
        # Make specified transformer blocks trainable
        for idx in trainable_indices:
            if 0 <= idx < num_blocks:
                for param in transformer_blocks[idx].parameters():
                    param.requires_grad = True
                print(f"Made transformer block {idx} trainable")
            else:
                print(f"Warning: Invalid block index {idx}, skipping")
        
        # Always make final layer norm trainable
        if hasattr(vision_model, 'ln_post'):
            for param in vision_model.ln_post.parameters():
                param.requires_grad = True
            print(f"Made final layer norm trainable")
        
        # Make final projection trainable - handle both module and parameter cases
        if hasattr(vision_model, 'proj'):
            if isinstance(vision_model.proj, nn.Parameter):
                vision_model.proj.requires_grad = True
                print(f"Made final projection parameter trainable")
            elif hasattr(vision_model.proj, 'parameters'):
                for param in vision_model.proj.parameters():
                    param.requires_grad = True
                print(f"Made final projection module trainable")
    else:
        print("Could not identify transformer blocks in the model structure")
...
if __name__ == '__main__':
    ...
    f"trainable_last{args.trainable_layers}_"
