...
parser.add_argument('--alpha', type=float, default=0.99, help='EMA decay rate for updating running mean in MINE estimators')
...
parser.add_argument('--use_sn', type=str2bool, default=False, help='Whether to use spectral normalization on discriminator')
...
class ComputeInfoGapLossWrapper:
    ...
    def __call__(self, embedding_adv, targets, step=None, total_steps=None):
        ...
        # 최종 손실 계산
        loss = loss_info_term + self.lambda_val * (10 * step / total_steps)* adaptive_lambda * loss_kld_term
...
class Mine(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # EMA for the second term (partition function)
        exp_mean_batch = torch.exp(t_shuffled).mean()
        if self.running_mean is None:
            self.running_mean = exp_mean_batch
        else:
            self.running_mean = (1 - self.alpha) * self.running_mean.detach() + self.alpha * exp_mean_batch
...
class WeightedMine(nn.Module):
    def __init__(self, T_network, discriminator, alpha=0.01):
        super().__init__()
        self.alpha = alpha
        self.T = T_network
        self.discriminator = discriminator
        self.running_mean = None # EMA를 위한 running_mean 추가
    ...
    def forward(self, x, y, w):
        ...
        w_shuffled = compute_w(x, y_shuffled, self.discriminator).detach()
        
        # EMA for the second term
        exp_mean_batch = (w_shuffled * torch.exp(t_shuffled)).mean()
        if self.running_mean is None:
            self.running_mean = exp_mean_batch
        else:
            self.running_mean = (1 - self.alpha) * self.running_mean.detach() + self.alpha * exp_mean_batch
        
        second_term = torch.log(self.running_mean + EPS)  # EMA 적용
        mi_estimate = first_term - second_term * args.second_term_coeff
        
        # intermediate 값도 반환
        return mi_estimate, t.detach(), w.detach(), t_shuffled.detach(), wt.detach(), exp_mean_batch.detach(), first_term.detach(), second_term
...
class MLPDiscriminator(nn.Module): # Renamed from Discriminator
    def __init__(self, x_dim, y_dim, use_sn=False):
        super().__init__()
        
        layer1 = nn.Linear(x_dim + y_dim, 512)
        layer2 = nn.Linear(512, 512)
        layer3 = nn.Linear(512, 1)

        if use_sn:
            layer1 = nn.utils.spectral_norm(layer1)
            layer2 = nn.utils.spectral_norm(layer2)
            layer3 = nn.utils.spectral_norm(layer3)
...
def compute_gradient_norm(loss, target_params):
    """
    특정 손실에 대해 주어진 파라미터의 그래디언트 놈을 계산합니다.

    Args:
        loss (torch.Tensor): 손실 값
        target_params (Iterable[torch.nn.Parameter]): 그래디언트를 계산할 파라미터

    Returns:
        torch.Tensor: 그래디언트 놈 값
    """
    # 그래디언트 계산
    grads = torch.autograd.grad(
        outputs=loss,
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        # E_q_wlog_w 계산 (다양한 regul 옵션 지원)
        if args.regul == 'rkl':
            E_q_wlog_w = (w_q * torch.log(w_q + EPS)).mean()
        elif args.regul == 'logchi':
            chi_squared = ((w_q - 1)**2).mean()
            E_q_wlog_w = torch.log(1 + chi_squared + EPS)  # 안정성을 위해 EPS 추가
        elif args.regul == 'chi':
            E_q_wlog_w = ((w_q - 1)**2).mean()
        elif args.regul == 'neymanchi':
            E_q_wlog_w = (((w_q - 1)**2) / w_q).mean()
        else:
            raise ValueError(f"Unknown regul type: {self.args.regul}")
        ...
        if args.lambda_type == 'normal':
            adaptive_lambda = args.lambda_val
        elif args.lambda_type == 'annealing':
            adaptive_lambda = args.lambda_val * (step_total / args.steps)
        elif args.lambda_type == 'grad_aware':
            # 그래디언트 계산을 위한 판별자 파라미터 선택
            if args.disc_arch == 'mlp':
                disc_params_for_grad = list(unwrap_model(discriminator).layers[-1].parameters())
            elif args.disc_arch == 'bilinear':
                disc_params_for_grad = list(discriminator.parameters())
            else:
                raise ValueError(f"Unknown disc_arch for gradient norm: {args.disc_arch}")

            # 그래디언트 놈 계산
            grad_info_norm = compute_gradient_norm(loss_info_term, disc_params_for_grad)
            grad_kld_norm = compute_gradient_norm(loss_kld_term, disc_params_for_grad)

            # 메트릭 기록
            metrics['grad_info_norm'].append(grad_info_norm.item())
            metrics['grad_kld_norm'].append(grad_kld_norm.item())

            # 람다 값을 계산하되, 이 값 자체는 상수로 취급하여 역전파를 차단
            adaptive_lambda = (grad_info_norm / (grad_kld_norm + EPS)).detach()
        else:
            raise ValueError(f"Unknown lambda_type: {args.lambda_type}")
        
        # loss_phi_abs의 디폴트값은 false임 --> loss_phi_abs가 True일 경우, loss_phi에 절대값을 취함
        # parsing 잘 되어있는지 확인해야 함
        if not args.loss_phi_abs:
            loss_phi_infogap = loss_info_term +  adaptive_lambda * loss_kld_term
        else:
            loss_phi_infogap = (loss_info_term + adaptive_lambda * loss_kld_term)**2
       ...
       #metrics['w_q'].append(w_q_quartile_mean)
        ...
        lambda_to_print = adaptive_lambda.item() if isinstance(adaptive_lambda, torch.Tensor) else adaptive_lambda
        print(f"[Step {step_total}] loss_phi: {loss_phi.item():.4f}, loss_d: {loss_d.item():.4f} | Lambda: {lambda_to_print:.4f} | Model LR: {optimizer.param_groups[0]['lr']:.6f} | Disc LR: {optimizer_d.param_groups[0]['lr']:.6f}")
        ...
        print(f"Weighted_MI: {weighted_mi.item():.4f}, Standard_MI: {standard_mi.item():.4f}, w_p_mean: {mean_w_p:.4f},w_q_mean: {mean_w_q:.4f}")
...
def main():
    ...
    if args.disc_arch == 'mlp':
        ...
        discriminator = MLPDiscriminator(x_dim, y_dim, use_sn=args.use_sn).to(device)
        if args.use_sn:
            print("Applying Spectral Normalization to MLPDiscriminator.")
        else:
            # SN은 자체 초기화 로직이 있으므로, 사용하지 않을 때만 가중치 초기화 적용
        ...
        if args.use_sn:
            print("Applying Spectral Normalization to BilinearDiscriminator.")
            if hasattr(discriminator, 'W'): # Full-rank case
                discriminator = nn.utils.spectral_norm(discriminator, name='W')
            elif hasattr(discriminator, 'U') and hasattr(discriminator, 'V'): # Low-rank case
                discriminator = nn.utils.spectral_norm(discriminator, name='U')
                discriminator = nn.utils.spectral_norm(discriminator, name='V')
        # BilinearDisc는 자체 init_weights를 호출하므로, discriminator.apply(weights_init)는 필요하지 않음.
    else:
    ...
    if args.discriminator_pretrain_steps > 0:
        ...
        while step_d < args.discriminator_pretrain_steps:
            for data, targets in dataloader:
                ...
                loss_d_real = F.relu(1.0 - logits_real).mean()
                loss_d_fake = F.relu(1.0 + logits_fake).mean()
                loss_d_hinge = loss_d_real + loss_d_fake
    ...
    mi_estimator_weighted = WeightedMine(T_network,discriminator=discriminator, alpha=args.alpha).to(device)
    ...
    if args.shell_script_path:
        try:
            if os.path.exists(args.shell_script_path):
                ...
                # Optionally, save args.txt as a fallback if shell script not found or not provided
