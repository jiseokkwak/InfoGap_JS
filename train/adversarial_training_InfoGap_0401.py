...
class ComputeInfoGapLossWrapper:
    ...
    def __call__(self, embedding_adv, targets=None):
        ...
        w_pair = D_psi_pair / (1.0 - D_psi_pair + EPS)
        t_pair = self.T_network(adv_flat, y_flat)
        t_matrix = t_pair.view(B, B)  # [B, B], t_ij = T(x_i, y_j)
        w_matrix = w_pair.view(B, B)
        exp_t_matrix = torch.exp(t_matrix)
        w_exp_t_matrix = w_matrix * exp_t_matrix
        numerator_per_row_i2t = w_exp_t_matrix.sum(dim=1)  # 이미지 x_i에 대해 모든 y_j 합산
        denominator_per_row_i2t = exp_t_matrix.sum(dim=1)
        log_ratios_i2t = torch.log(numerator_per_row_i2t / (denominator_per_row_i2t + EPS) + EPS)
        secondterm_i2t = -1 * log_ratios_i2t.mean()  # L_i2t

        # 3) L_t2i 계산 (새로 추가)
        numerator_per_row_t2i = w_exp_t_matrix.sum(dim=0)  # 텍스트 y_j에 대해 모든 x_i 합산
        denominator_per_row_t2i = exp_t_matrix.sum(dim=0)
        log_ratios_t2i = torch.log(numerator_per_row_t2i / (denominator_per_row_t2i + EPS) + EPS)
        secondterm_t2i = -1 * log_ratios_t2i.mean()  # L_t2i

        # 4) 두 방향의 손실 평균
        secondterm = 0.5 * (secondterm_i2t + secondterm_t2i)
