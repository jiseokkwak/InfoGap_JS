...
parser.add_argument('--mi_estimator_type', type=str, default='dv', choices=['dv', 'infonce'], help='Type of MI estimator to use.')
parser.add_argument('--use_self_normalizing_is', type=str2bool, default=False, help='Whether to use self-normalizing importance sampling in WeightedMine for InfoNCE.')
...
class T(nn.Module):
    ...
    def forward(self, x, y):
        # Ensure x and y are 2D for cosine_similarity if they are not already
        if x.ndim > 2 and y.ndim > 2 and x.shape[0] == y.shape[0] and x.shape[1] == y.shape[1]: # B, B, Dim case
             orig_x_shape = x.shape
             x = x.reshape(-1, x.shape[-1])
             y = y.reshape(-1, y.shape[-1])
             cos_sim = F.cosine_similarity(x, y, dim=1, eps=EPS)
             return (cos_sim/args.temperature).view(orig_x_shape[0], orig_x_shape[1])
        elif x.ndim == 2 and y.ndim == 2: # Standard B, Dim case
             cos_sim = F.cosine_similarity(x, y, dim=1, eps=EPS)
...
class Mine(nn.Module):
    def __init__(self, T_network, estimator_type='dv'): # Removed alpha
        super().__init__()
        self.T = T_network
    ...
    def forward(self, x, y):
        if self.estimator_type == 'dv':
            ...
            exp_mean_shuffled = torch.exp(t_shuffled).mean()
            second_term_val = torch.log(exp_mean_shuffled + EPS)

            mi_estimate = first_term - second_term_val * args.second_term_coeff # As per user's snippet (mi_estimate = first_term)
            return mi_estimate, t.detach(), t_shuffled.detach(), first_term.detach(), second_term_val

        elif self.estimator_type == 'infonce':
            batch_size = x.size(0)
            dim = x.size(1)

            positive_scores = self.T(x, y) 

            x_expanded = x.unsqueeze(1).expand(batch_size, batch_size, dim)
            y_expanded = y.unsqueeze(0).expand(batch_size, batch_size, dim)
            
            all_scores = self.T(x_expanded.reshape(batch_size*batch_size, dim), 
                                y_expanded.reshape(batch_size*batch_size, dim)).view(batch_size, batch_size)

        
            mi_estimate = positive_scores.mean()                   # ❌ 분모항 없음
            log_sum_exp = torch.logsumexp(all_scores, dim=1)       # [B]
        
            # InfoNCE: 1/B * Σ_i (s_ii - log Σ_j e^{s_ij})
            mi_estimate = (positive_scores - log_sum_exp).mean()   # ✅ 분모 포함

            # 로그·디버깅용 값들

            first_term_detached  = positive_scores.mean().detach()
            second_term_detached = log_sum_exp.mean().detach()
            # Modified mi_estimate to be only the first term, similar to 0304.py's Mine
            ...
            return mi_estimate, \
                   ...
                   all_scores.detach(), \
                   first_term_detached, \
                   second_term_detached
        else:
            raise ValueError(f"Unknown mi_estimator_type: {self.estimator_type}")

class WeightedMine(nn.Module):
    """
    Mutual-Information estimator supporting
    - Donsker-Varadhan(DV)
    - InfoNCE   (denominator 포함!)
    Optional self-normalizing importance sampling (SNIS) ↔ plain weighted 평균
    """
    def __init__(self,
                 T_network: nn.Module,
                 estimator_type: str = "dv",           # "dv" | "infonce"
                 use_self_normalizing: bool = True):   # SNIS 켜기/끄기
        super().__init__()
        self.T = T_network
        self.estimator_type = estimator_type.lower()
        self.use_self_normalizing = use_self_normalizing

    def forward(self, x: torch.Tensor, y: torch.Tensor, w: torch.Tensor):
        """
        Args
        ----
        x : (B, d)   image/adv embedding
        y : (B, d)   text embedding (positive pair aligned)
        w : (B,)     importance weight  w = Dψ / (1−Dψ)

        Returns
        -------
        mi_estimate, pos_scores, w, all_scores, w*pos, w*all,
        first_term_detached, second_term_detached
        """
        B, d = x.size()
        ...
        if self.estimator_type == "dv":
            t_pos = self.T(x, y)                # (B,)
            y_shuff = y[torch.randperm(B)]
            w_shuff = w[torch.randperm(B)]
            t_shuff = self.T(x, y_shuff)

            first_term  = (w * t_pos).mean()
            second_term = torch.log( torch.exp(w_shuff * t_shuff).mean() + EPS )

            mi = first_term - second_term * args.second_term_coeff

            return (mi, t_pos.detach(), w.detach(), t_shuff.detach(),
                    (w * t_pos).detach(), (w_shuff * t_shuff).detach(),
                    first_term.detach(), second_term.detach())

        # ----- InfoNCE estimator ---------------------------------------------
        elif self.estimator_type == "infonce":
            # positive scores s_ii
            s_pos = self.T(x, y)                                    # (B,)

            # all pair scores s_ij
            x_exp = x.unsqueeze(1).expand(B, B, d)                  # (B,B,d)
            y_exp = y.unsqueeze(0).expand(B, B, d)
            s_all = self.T(x_exp.reshape(B*B, d),
                           y_exp.reshape(B*B, d)).view(B, B)        # (B,B)

            # InfoNCE per-sample term: s_ii − log∑_j exp s_ij
            log_den = torch.logsumexp(s_all, dim=1)                 # (B,)
            info_terms = s_pos - log_den                            # (B,)

            # weighting: self-normalizing IS vs. plain 평균
            if self.use_self_normalizing:
                mi = (w * info_terms).sum() / (w.sum() + EPS)
            else:
                mi = (w * info_terms).sum() / B

            # detach helpers for 로그/모니터링
            return (mi,
                    s_pos.detach(),
                    w.detach(),
                    s_all.detach(),
                    (w * s_pos).detach(),
                    (w.unsqueeze(1) * s_all).detach(),
                    info_terms.mean().detach(),        # first-term proxy
                    log_den.mean().detach())           # second-term proxy
...
def main():
    ...
    # Pass estimator_type and use_self_normalizing from args
    mi_estimator_weighted = WeightedMine(
        T_network, 
        estimator_type=args.mi_estimator_type,
        use_self_normalizing=args.use_self_normalizing_is  # Pass the new flag
    ).to(device)
    mi_estimator_standard = Mine(T_network, estimator_type=args.mi_estimator_type).to(device)
