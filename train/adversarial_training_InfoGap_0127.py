...
#second term의 계산을 min_ij 1/w_q_ij로 변경함 
...
class ComputeInfoGapLossWrapper:
    ...
    def __call__(self, embedding_adv, targets=None):
        ...
        numerator = torch.sum(w_pair * torch.exp(t_pair))
        denom = torch.sum(torch.exp(t_pair))
        secondterm = -torch.log(numerator / (denom + EPS) + EPS)
        ...
        loss = weighted_mi - standard_mi + self.lambda_val * (E_q_wlog_w) + secondterm
        return loss
