...
class WeightedMine(nn.Module):
    ...
    def forward(self, x, y, w):
        ...
        #w_shuffled = w[perm]
        with torch.no_grad():
            w_shuffled = compute_w(x, y_shuffled, self.discriminator).detach()  # w_shuffled 계산
        t_shuffled = self.T(x, y_shuffled)
        ...
        #second_term = torch.log(exp_mean + EPS)  # 직접 exp_mean 사용
        second_term = torch.log((w_shuffled * torch.exp(t_shuffled)).mean() + EPS)
        mi_estimate = first_term - second_term * args.second_term_coeff
...
def main():
    ...
    mi_estimator_weighted = WeightedMine(T_network, discriminator, alpha=args.alpha).to(device)
