...
#discriminator 안쓰는 대신 logp = cos_sim 으로 대체
import sys
import os
import shutil
import time
...
parser.add_argument('--scheduler_type', type=str, default='cosine', help='Scheduler type (e.g., cosine, linear)')
...
class ComputeInfoGapLossWrapper:
    def __init__(self, y, mi_estimator_weighted, mi_estimator_standard, lambda_val, T_network, return_components=True):
        """
        y: 텍스트 임베딩, shape [B, d]
    ...
    def __call__(self, embedding_adv, embedding_orig):
        # 1) w_q 계산
        log_w_q = (embedding_orig * self.y).sum(dim=1) - (embedding_adv * self.y).sum(dim=1)
        log_w_q = torch.clamp(log_w_q, min=-10, max=10)  # 수치 안정성을 위해
        w_q = torch.exp(log_w_q)
        E_q_wlog_w = (w_q*torch.log(w_q + 1e-8)).mean()

        # 2) secondterm 계산 (discriminator 없이)
        B = embedding_adv.size(0)
        # w_pair_matrix 계산
        orig_y_inner = (embedding_orig * self.y).sum(dim=1)  # [B]
        adv_y_inner = torch.matmul(embedding_adv, self.y.T)  # [B, B]
        log_w_pair_matrix = orig_y_inner.unsqueeze(0) - adv_y_inner  # [B, B]
        log_w_pair_matrix = torch.clamp(log_w_pair_matrix, min=-10, max=10)  # 안정성
        w_pair_matrix = torch.exp(log_w_pair_matrix)
        w_matrix = w_pair_matrix
        ...
        log_ratios = torch.log(numerator_per_row / (denominator_per_row + 1e-8) + 1e-8)
...
#             nn.Dropout(args.dropout),
#             nn.LeakyReLU(args.leaky_relu, inplace=True),
#             spectral_norm(nn.Linear(512, 1))  # 출력 레이어
#         )
...
def train_one_epoch(
    ...
    embedding_text_labels_norm, args, epoch,  
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        if args.attack == 'pgd':
            data_adv = pgd(
                forward=model,
                loss_fn=lambda adv,_: loss_inner_wrapper(adv, embedding_orig),
                data_clean=data,
                targets=targets,
                norm=args.norm,
                eps=args.eps,
        ...
        log_w_q = (embedding_orig * y).sum(dim=1) - (embedding_adv * y).sum(dim=1)
        w_q = torch.exp(log_w_q)
        w_p = w_q
        E_q_wlog_w = (w_q*torch.log(w_q + 1e-8)).mean()
        ...
        loss_dict = loss_inner_wrapper(embedding_adv, embedding_orig)
        ...
        optimizer.step()
        if args.scheduler_type == 'cyclicLR':
            scheduler.step()
        else:
       ...
       #metrics['w_q'].append(w_q_quartile_mean)
        ...
        print(f"[Step {step_total}] loss_phi: {loss_phi.item():.4f}")
        ...
        del w_p, w_q, E_q_wlog_w, weighted_mi, standard_mi, loss_dict, loss_phi
        del cos_sim, racc, acc, y_normed, cos_sim_image_text, embedding_diff, l2_norm
...
def plot_metrics(metrics, checkpoint_dir, step):
    plt.figure(figsize=(18, 18))  # 6행 2열 subplot으로 조정
    ...
    # 8) Gradient Norm for Weighted MI
    non_zero_grad_weighted = [v for i, v in enumerate(metrics.get('grad_weighted', [])) if v != 0]
    ...
    # 9) Gradient Norm for Standard MI
    non_zero_grad_standard = [v for i, v in enumerate(metrics.get('grad_standard', [])) if v != 0]
    ...
    # 10) Gradient Norm for E_q_wlog_w
    non_zero_grad_eq = [v for i, v in enumerate(metrics.get('grad_eq', [])) if v != 0]
    if non_zero_grad_eq:
        plt.subplot(6, 2, 10)
    ...
    # 11) Gradient Norm for Second Term
    non_zero_grad_second = [v for i, v in enumerate(metrics.get('grad_second', [])) if v != 0]
...
def main():
    if not args.experiment_name:
        args.experiment_name = (
            ...
            f"lambda{args.lambda_val}_leaky{args.leaky_relu}_"
            f"eps_{args.eps}_"
    ...
    if args.scheduler_type == 'cosine':
        ...
        scheduler = torch.optim.lr_scheduler.CyclicLR(
            optimizer,
            base_lr=args.lr / 10,    # adjust base_lr as needed
            max_lr=args.lr,
            step_size_up=args.warmup,
            mode='triangular',
