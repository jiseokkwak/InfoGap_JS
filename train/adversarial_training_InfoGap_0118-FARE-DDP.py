...
if 'RANK' in os.environ:
    ...
    local_rank = 0  # single‑process fallback
...
def plot_metrics(metrics, checkpoint_dir, step):
    ...
    # 3) Gradient Norms
    plt.subplot(4, 2, 3)
    plt.plot(metrics['grad_info_norm'], label='Grad Info Norm', color='purple')
    plt.plot(metrics['grad_kld_norm'], label='Grad KLD Norm', color='brown')
    plt.xlabel('Steps')
...
def main():
    ...
    eval_sampler  = torch.utils.data.distributed.DistributedSampler(dataset_eval, shuffle=False)
    dataloader = DataLoader(dataset, batch_size=args.batch_size, sampler=train_sampler, num_workers=16, drop_last=True)
    dataloader_eval = DataLoader(dataset_eval, batch_size=args.batch_size, sampler=eval_sampler, num_workers=16, drop_last=True)
    ...
    model_orig.to(device)
    model = ClipVisionModel(model=model.visual, args=args, normalize=normalize).to(device)
    ...
    if local_rank == 0:
        print(f"DDP initialized on rank {dist.get_rank()} (world size {dist.get_world_size()})")
    ...
    metrics = {
        ...
        'D_q': [],
        'E_q_wlog_w': [],
        'grad_info_norm': [],       # Gradient norm for Info term
        'grad_kld_norm': []         # Gradient norm for KLD term
    }
...
if __name__ == '__main__':
    ...
    # When using DDP, prevent multiple processes from writing to stdout simultaneously
    if 'RANK' in os.environ:
