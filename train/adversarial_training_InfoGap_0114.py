...
def main():
    ...
    dataloader_eval = DataLoader(dataset_eval, batch_size=args.batch_size, shuffle=False, num_workers=16, drop_last=False)
    ...
    optimizer_d = torch.optim.Adam(discriminator.parameters(), lr=args.disc_lr_coeff * args.lr, betas=(args.beta1, 0.999), weight_decay=0.1*args.wd)
    total_discriminator_steps = args.discriminator_pretrain_steps + args.steps
    scheduler_d = cosine_lr(optimizer_d, args.lr, args.discriminator_pretrain_warmup, total_discriminator_steps)
    ...
    while step_total < args.steps:
        print(f"Starting main training epoch {epoch+1}...")
...
if __name__ == '__main__':
    ...
    args.finetuned_model_name = f'{args.clip_model_name}_{args.pretrained}_{args.dataset}_{args.loss}_{args.experiment_name}_{random_str}'
